...
import { stat } from 'fs';
import { filterDM, filterChannelType, filterNoBotMessages } from '../middleware/index';
...
const messaging = async (app: App) => {
    app.message( filterChannelType("im"), filterNoBotMessages,  async ({ message, body, say }) => {
        ...
        if (state === "creatingEntry") {
            ...
            say("I'm not sure I understand. If you want to create an entry, run `/entry`!")
        }
    })
}
